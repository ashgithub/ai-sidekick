...
def wait_for_run(*, base_url: str, run_id: str, timeout_seconds: float = 120) -> dict[str, object]:
    ...
    terminal = {"completed", "failed", "not_found", "stale_source"}
...
def main(argv: list[str] | None = None) -> int:
    ...
    prompt = (
        build_zsh_prompt(text)
        if args.prompt_kind == "zsh"
        else build_ai_tools_prompt(text=text, app_context=args.app_context, nudge=nudge)
    )
    try:
    ...
    if args.wait:
        ...
        output = str(run.get("response_text", ""))
